...
import { Sparkles, ArrowRight, ShieldCheck, Flame, Lock, Calendar, CheckCircle2 } from "lucide-react";
import VisibilityDiagnosisModal from "./VisibilityDiagnosisModal";
...
export default function PalvelutHero() {
  ...
  return (
    <>
      <section className="relative py-12 md:py-20 bg-[var(--bg)] text-[var(--text)] overflow-hidden selection:bg-[var(--accent)]/20">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center text-center">
          ...
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4 w-full sm:w-auto">
            ...
            {/* Primary CTA: Free 45-min Diagnosis */}
            <button
              onClick={() => setModalOpen(true)}
              className="btn btn--primary btn--lg w-full sm:w-auto text-sm sm:text-base font-bold shadow-xl flex items-center justify-center gap-2.5 cursor-pointer"
            >
              <Calendar className="w-5 h-5 text-[var(--accent-ink)]" />
              <span>Varaa ilmainen Näkyvyysdiagnoosi (45 min)</span>
            </button>
            ...
            >
              <span>Katso Perustajahinta (499 €)</span>
      ...
      {/* Modal Trigger */}
      <VisibilityDiagnosisModal isOpen={modalOpen} onClose={() => setModalOpen(false)} />
    </>
  );
}
