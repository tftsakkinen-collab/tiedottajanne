...
import { BookOpen, ShieldCheck, Search, CheckCircle2 } from "lucide-react";
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-[var(--border)] pb-8">
          <div className="space-y-3.5">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-[var(--text)] font-display">
              SOTE-alan <span className="text-gradient-cyan">sanasto &amp; lääketieteellinen laatu</span>
            </h2>
          ...
          <div className="p-4 sm:p-5 rounded-2xl glass border border-[var(--border)] flex items-center gap-3.5 shrink-0">
            <ShieldCheck className="w-8 h-8 text-[var(--accent)] shrink-0" />
            <div className="text-xs">
              <span className="font-bold text-[var(--text)] block font-display">Medical Review Verified</span>
              <span className="text-[var(--muted)]">Valvira / Terhikki -rekisteröity OMT-Fysioterapeutti Janne Säkkinen</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 rounded-full bg-[var(--surface)] border border-[var(--border)] text-xs text-[var(--text)] placeholder-[var(--muted)] focus:outline-none focus:border-[var(--accent)] focus-visible:ring-2 focus-visible:ring-[var(--accent)] transition-colors min-h-[44px]"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {categories.map((cat) => (
              <button
                ...
                className={`min-h-[44px] px-4 py-2 rounded-full text-xs font-semibold transition-all cursor-pointer flex items-center justify-center ${
                  selectedCat === cat
                    ? "bg-[var(--accent)] text-[var(--accent-ink)] font-bold shadow-md"
                    : "bg-[var(--surface)] text-[var(--muted)] border border-[var(--border)] hover:border-[var(--accent)]/40 hover:text-[var(--text)]"
                }`}
              >
                {cat}
              </button>
